itbmobile.Employee = Backbone.Model.extend({

    initialize:function () {
        this.reports = new itbmobile.ReportsCollection();
        this.reports.parent = this;
    },

    sync: function(method, model, options) {
        if (method === "read") {
            itbmobile.store.findById(parseInt(this.id), function (data) {
...
itbmobile.EmployeeCollection = Backbone.Collection.extend({

    model: itbmobile.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            itbmobile.store.findByName(options.data.name, function (data) {
                options.success(data);
            });
        }
    }
...
itbmobile.ReportsCollection = Backbone.Collection.extend({

    model: itbmobile.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            itbmobile.store.findByManager(this.parent.id, function (data) {
                options.success(data);
            });
        }
    }
...
itbmobile.MemoryStore = function (successCallback, errorCallback) {
...
itbmobile.store = new itbmobile.MemoryStore();